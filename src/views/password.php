...
if (!empty($_POST)) {
    ...
    $check1 = $conn->real_escape_string($_POST["check1"]);
    $check2 = $conn->real_escape_string($_POST["check2"]);
    ...
    if ($check1 ==  $check2) { //Step 1: check if passwor confirmation is OK
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            $stmt->bind_param('i', $id); //Send parameter to server
            $stmt->execute();
            $result = $stmt->get_result();

            if ($result->num_rows == 1) { //Step 3: Check if query is OK
